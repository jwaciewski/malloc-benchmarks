...
#include "laballoc-install/include/memory_mgmt.h"
#include "laballoc-install/include/types.h"
#include <errno.h>
#include <math.h>
#include <pthread.h>
#include <signal.h>
...
static void alarm_handler(int signum) { timeout = true; }
...
static void usage(const char *name) {
  fprintf(stderr, "%s: [<num_threads>] <alloc-strat>>\n ", name);
  exit(1);
}
...
int main(int argc, char **argv) {
  ...
  if (argc == 2) {
    num_threads = 1;
    if (!strcmp("first_fit", argv[1])) {
      set_alloc_function(FIRST_FIT);
    } else if (!strcmp("next_fit", argv[1])) {
      set_alloc_function(NEXT_FIT);
    } else if (!strcmp("best_fit", argv[1])) {
      set_alloc_function(BEST_FIT);
    } else if (!strcmp("worst_fit", argv[1])) {
    ...
    num_threads = ret;
    if (!strcmp("first_fit", argv[2])) {
      set_alloc_function(FIRST_FIT);
    } else if (!strcmp("next_fit", argv[2])) {
      set_alloc_function(NEXT_FIT);
    } else if (!strcmp("best_fit", argv[2])) {
      set_alloc_function(BEST_FIT);
    } else if (!strcmp("worst_fit", argv[2])) {
      set_alloc_function(WORST_FIT);
    } else {
      usage(argv[0]);
    }
